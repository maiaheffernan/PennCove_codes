...
Configures regime sampling on an RBR Concerto via RS-232 serial communication
on macOS. This is for the Convertos that will be connected to the Fermatta battery packs on the WireWalkers.
Commands match the Logger2 Command Reference (RBR0001963revB).

Sampling regime: logger will be asleep until its pressure reading detects it 19 meters underwater and rising on 
an upcast. The logger will then sampling continuously until it reaches the surface, at which point it will stop 
sampling and go back to sleep until the next upcast.
...
def main():
    print("=" * 55)
    print("  RBR Concerto — Upcast Regime Sampling Configuration Script")
